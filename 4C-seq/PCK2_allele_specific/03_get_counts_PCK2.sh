...
## create a counts file(.bedGraph) with number of reads per RE fragment using the sam_bedGraph.sh file with the argument for the directory where .SAM files can be found. This will make a .bedGraph file for ALL *aligned.sam files in the folder.
#bash ~/bin/4Cker/sam_bedGraph.sh ${DIR}/4Cker/aligned/
...
for file in *.bam;
do
	coverageBed -a ${DIR}/4Cker/design/dpnII_hpych4v.rmap -b $file > ../counts/$(echo $file | sed 's/.bam/.counts/g');
done
