...
for file in $(cat THBS1_SNPs.counts);
...
## the same but not removing those frags now. Want to check the normalisation.
for file in $(cat THBS1_SNPs.counts);
do cat $file | awk 'OFS="\t"{print $1,$2,$3,$5}' > $(echo $file | sed 's/.counts/.bedGraph/g');
done
