...
setwd("/Users/hrayjone/Documents/Data/AS_ATAC/")
alleles <- fread("./snps/closest_peaks_5kb.bed")
alleles <- fread("~/HRJ_monocytes/AS_ATAC/BaseQTL/snps/closest_peaks_5kb.bed")
...
setwd("~/HRJ_monocytes/AS_ATAC/BaseQTL/phASER_out")
...
### We seem to have more reads now :)

######## Write files.

setwd("~/HRJ_monocytes/AS_ATAC/BaseQTL/allele_counts")
...
######## Write file
getwd()
setwd("/Users/hrayjone/Documents/Data/AS_ATAC/allele_counts/")
...
multi <- fread("/Users/hrayjone/Documents/Data/AS_ATAC/snps/multialleleic_snps.txt",
               header = FALSE)
#multi <- fread("~/HRJ_monocytes/genotyping/imputed/hg38_filtered/multialleleic_snps.txt",
#              header = FALSE)
atac_matrix <- fread("/Users/hrayjone/Documents/Data/AS_ATAC/allele_counts/mono_34reps_ATAC_allele_counts_phASER.txt")
...
nrow(to_test) # 2020442
length(unique(to_test[, SNP])) # 1010221 # Good!

### 2.  Make sure the alleles do not have any strand flips. the SNP IDs should match the hg38 strand.
genosnps <- fread("/Users/hrayjone/Documents/Data/AS_ATAC/snps/all.rsq3.maf05.hg38.dose.finalRenamed.newIDs.sites.vcf")
...
# Check if the SNP name is always in the HRC file
toEdit[!SNP %in% genos$ID] # None.
...
fwrite(no_multi_alleles, file = "./mono_34reps_ATAC_allele_counts_phASER_noMulti_hg38Strand.txt", 
...
#### Save
fwrite(no_multi_totals, file = "./mono_34reps_ATAC_total_counts_noMulti_hg38Strand.txt", 
       sep = "\t", quote = FALSE, row.names = FALSE)

############# Will do same for CHiC

#### Double check them
alleles <- fread("./mono_34reps_ATAC_allele_counts_phASER_noMulti_hg38Strand.txt")
totals <- fread("./mono_34reps_ATAC_total_counts_noMulti_hg38Strand.txt")
#### 
